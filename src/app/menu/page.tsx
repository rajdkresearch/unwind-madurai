...
function MenuItemCard({ item }: { item: MenuItem }) {
  ...
  return (
    ...
    >
      {/* Image placeholder */}
      <div className="w-24 h-24 rounded-xl bg-gradient-to-br from-[#1A1A2E] to-[#0D0D1A] shrink-0 overflow-hidden flex items-center justify-center text-3xl border border-white/5 relative">
        {item.is_veg ? '🥗' : '🍖'}
        {item.is_bestseller && (
    ...
    <div className="min-h-screen pt-24 pb-32 max-w-7xl mx-auto px-4 sm:px-6">
      ...
      {cartCount === 0 && (
        <div className="fixed bottom-6 right-6 z-50">
          <button onClick={() => router.push('/checkout')}
            className="flex items-center gap-2 px-5 py-3 bg-[#1A1A28] border border-gold/20 text-white/60 rounded-xl hover:border-gold/40 hover:text-white transition-all text-sm">
